...
def GenerateConfig(context):
    ...
    resources = [{
        ...
        'properties': {
            ...
            'machineType': '/zones/'+context.properties['zone']+'/machineTypes/'+context.properties['proxy-vm-type'],
            'canIpForward': True,
            'tags': {
                'items': [
            ...
            'metadata': {
                'items': [{
                    'key': 'startup-script',
                    'value': ''+getBootstrapAsArray(context.env['project']+':'+context.properties['region']+':$(ref.'+'hive-metastore-inst-'+context.properties['deployment-suffix']+'.name)')
                }]
            },
            'serviceAccounts': [
                {
                    'email': '$(ref.cloud-sql-proxy-service-acc.email)',
